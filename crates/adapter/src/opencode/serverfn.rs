...
#[cfg(test)]
mod tests {
    use crate::opencode::serverfn::{extract_json_value, parse_r_response, solidjs_to_json};

    #[test]
    fn test_parse_workspaces_response() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let result: Vec<serde_json::Value> = parse_r_response(text)?;
    ...
    #[test]
    fn test_extract_json_array() -> Result<(), Box<dyn std::error::Error>> {
        let s = r#"[1,2,3]rest"#;
        assert_eq!(extract_json_value(s).ok_or("missing array")?, "[1,2,3]");
        Ok(())
    }

    #[test]
    fn test_extract_json_object() -> Result<(), Box<dyn std::error::Error>> {
        let s = r#"{"a":1,"b":[2,3]}xyz"#;
        assert_eq!(
            extract_json_value(s).ok_or("missing object")?,
            r#"{"a":1,"b":[2,3]}"#
        );
        Ok(())
    }
